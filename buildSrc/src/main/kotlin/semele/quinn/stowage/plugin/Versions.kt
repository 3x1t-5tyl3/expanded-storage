package semele.quinn.stowage.plugin

import org.gradle.api.JavaVersion

object Versions {
    const val STOWAGE = "0.1.0"

    val JAVA = JavaVersion.VERSION_21
    const val MINECRAFT = "1.20.4"
    const val PARCHMENT = "1.20.4:2024.02.25"

    const val KOTLIN = "1.9.23"
    const val KOTLIN_COROUTINES = "1.8.0"
    const val KOTLIN_SERIALIZATION = "1.6.3"

    const val FABRIC_LOADER = "0.15.9"
    const val FABRIC_API = "0.96.11+1.20.4"
    const val FABRIC_KOTLIN = "1.10.19+kotlin.1.9.23"

    const val QUILT_LOADER = "0.23.1"
    const val QUILT_API = "9.0.0-alpha.7+0.96.11-1.20.4"
    const val QUILT_KOTLIN = "4.0.0+kt.1.9.23+flk.1.10.19"

    const val NEOFORGE = "20.4.225"
    const val KOTLIN_FOR_FORGE = "4.10.0"
}
